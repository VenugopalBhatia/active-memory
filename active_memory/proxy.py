...
class MemoryEngine:
    ...
    def __init__(self, store: SQLiteMemoryStore, embedding_provider: EmbeddingProvider, *, retrieval_config: RetrievalConfig | None = None, budget_config: BudgetConfig | None = None, storage_enabled: bool = True, strict: bool = False) -> None:
        self.store = store
        self.embedding_provider = embedding_provider
        self.counter = ApproximateTokenCounter()
        self.ingestor = MemoryIngestor(store, embedding_provider, self.counter, storage_enabled=storage_enabled)
...
def build_engine(config: ProxyConfig, *, retrieval_config: RetrievalConfig | None = None, budget_config: BudgetConfig | None = None) -> MemoryEngine:
    ...
    return MemoryEngine(store, provider, retrieval_config=retrieval_config, budget_config=budget_config, storage_enabled=config.storage_enabled, strict=config.strict_memory)
...
def main() -> None:
    ...
    parser.add_argument("--database", default="~/.active-memory/memory.db")
    parser.add_argument("--namespace", default="global")
    parser.add_argument("--embedder", choices=["local", "openai", "test"], default="local")
    ...
    config = ProxyConfig(args.upstream, None, args.host, args.port, args.database, args.namespace, args.embedder, args.embed_model, args.strict_memory, not args.disable_storage, args.verbose)
