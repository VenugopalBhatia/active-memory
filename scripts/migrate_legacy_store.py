    ...
    try:
        ...
        for index, item in enumerate(raw_messages):
            try:
                ...
                message_count += 1
            except Exception as exc:  # noqa: BLE001 - report every rejected legacy record
                skipped.append({"kind": "message", "index": index, "error": str(exc)})
        ...
        for index, item in enumerate(tuples):
            try:
                ...
                timestamp = datetime.fromtimestamp(float(item.get("created_at", 0)), tz=UTC)
                memory = Memory(
                ...
                skipped.append({"kind": "tuple", "index": index, "error": str(exc)})
    finally:
        store.close()
    return {
        "source": str(source),
